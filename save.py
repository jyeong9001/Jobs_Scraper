...
def save_to_file(jobs):
    ...
    writer.writerow(["Title", "Company", "Location",
                     "ApplyLink"])  # 엑셀 한 Cell 마다 구분해주려면 (지금처럼)한 데이터씩 ★콤마★로 구분지어서 List에 넣어줘야함 # row니깐 행-> 순서로 저장됨
    for job in jobs:
        writer.writerow(list(
            job.values()))  # writer.writerow(job.values()) # jobs에 저장된 {} 형태는 곧 dictionary형태라서, .values()로 값(values)만 불러올 수 있는 dictionary 기능 제공함 #list()로 감싸서 dict_values(~)로 묶인 상태를 없앰

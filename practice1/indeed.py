...
def extract_job(html):
    title = html.select_one(
        '.jobTitle>span').string  # 'new' 오류나는 title = html.find("h2", {"class":"jobTitle"}).find("span").string 대신 select_one 함수로!!
    # print(title)
    ...
    # location = html.select_one("pre > div").text  # 오류나는 html.find("div", {"class":"companyLocation"}).string 대신
    # select_one과 .text로!!!
    # job_id = html['data-jk']  # .parent 중요! 받아온 html의 부모 태그인 <a>에 [data-jk] 속성이 있음(사이트 업뎃 때문인듯)
